...
export function analyzeAnswer({ answer, question, role, transcriptSoFar = [], rubric = null, pressureScore = 50, responseSeconds = 0 }) {
  ...
  if (wordCount === 0) {
    return {
      ...
      faceVisibilityScore: null,
      engagementScore: null,
      visualMetricsAvailable: false,
      evidence: ['No answer text was submitted.'],
      strengths: [],
      weaknesses: ['No answer was provided.'],
      improvements: ['Answer the question with at least one specific example before submitting.'],
      fillerCount: 0,
      wordCount: 0,
      coveredPoints: [],
      missingPoints: rubric?.expectedPoints || [],
      starCoverage: {},
      responseSeconds,
      idealAnswer: 'A strong answer would directly address the question with context, your action, and a measurable result.',
      rewrite: ''
    };
  }
  ...
  if (wordCount < 8) {
    return {
      ...
      engagementScore: null,
      visualMetricsAvailable: false,
      evidence: [`Only ${wordCount} word${wordCount === 1 ? '' : 's'} were submitted.`],
      strengths: [],
      weaknesses: ['Answer was too short to evaluate meaningfully.'],
      improvements: ['Give a fuller response with context, action, and result.'],
      fillerCount,
      wordCount,
      coveredPoints: [],
      missingPoints: rubric?.expectedPoints || [],
      starCoverage: detectStarCoverage(lower),
      responseSeconds,
      idealAnswer: 'A model answer would include a brief situation, your specific action, and the outcome.',
      rewrite: 'I would expand this answer by explaining the situation, the decision I made, and the result.'
    };
  }
  ...
  // Delivery scores that can be estimated from the answer text/audio timing.
  // Camera-only scores are filled only when the client sends a live camera snapshot.
...
export function summarizeSession(session) {
  ...
  return {
    ...
    attentionScore: avg('attentionScore'),
    visualConfidenceScore: avg('visualConfidenceScore'),
    faceVisibilityScore: avg('faceVisibilityScore'),
    engagementScore: avg('engagementScore'),
    visualMetricsAvailable: entries.some((entry) => entry.analysis?.visualMetricsAvailable === true),
